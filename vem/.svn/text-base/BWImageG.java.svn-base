...
//System.out.println(cm.toString());
	...
	createHistogram("Histogram-GrayScale.pgm");
	save("Image-GrayScale.pgm",image);
}
...
public void filterImage () {
	...
	histogramArray = aFinder.getHistogram();
	...
	float[] der = new float[300];
	int[] point = new int[100];
	int numPoints=1;
	float percent = 0;	
	
	point[0] = 0;
	for (int k = 25; k < 230; k++) {
		der[k] = (histogramArray[k+1]-histogramArray[k])/1;		
		percent = (float)histogramArray[k]*100/(rows*columns-histogramArray[255]);			

		if(  (der[k-1] > 0) && (der[k] < 0) && percent >= 1.5  && (k > point[numPoints-1]+25)){		//Negative slope
			point[numPoints++] =  k;
		}
	}
	point[numPoints++] = 255;
	//TO DO change bkg color white to the color detected 
	for (int k = 0; k < 255; k++) {
		percent = (float)histogramArray[k]*100/(rows*columns-histogramArray[255]);
		System.out.println("histogram["+k+"]= "+histogramArray[k]+ "\t"+ percent+"%" + "\t\t\tDer="+der[k]);
	}
	
	System.out.println("Interest Points:" +numPoints);
	for(int n=0; n<numPoints ;n++){
		System.out.println(n+". histogram["+point[n]+"] ="+histogramArray[point[n]]);
	}
	
	int scale1 = (point[1] - point[0])/2;
	int scale2 = (point[numPoints-1] - point[numPoints-2])/2 ;
	...
	if(numPoints>2){
		...
		for(int n=1; n<numPoints-1 ;n++){			
			scale1 = (point[n] - point[n-1])/2 ;
			scale2 = (point[n+1] - point[n])/2 ;
			//if (scale > 25) scale = 25;
			System.out.println(n+". scale1= "+scale1 +". scale2= "+scale2);
			for (r = 0; r < rows; r++) {
				for (c = 0; c < columns; c++) {

					gray = image[r][c];
					if((gray >= point[n]-scale1) && (gray <= point[n]+scale2)){
					    image[r][c] = point[n];
					}
	...
	createHistogram("Histogram-Filter.pgm");
	save("ImageGrayScale-Filter.pgm",image);

}

//JJ
public void createHistogram(String name) {
		Background aFinder = new Background(image, rows, columns, 256);
		int margin = 10; //For the histogram image
		int[] histogramArray = aFinder.getHistogram();
		int[][] histogramImage = aFinder.makeHistogramImage(histogramArray, 256, margin);
		//System.out.println("Histogram image margins are "+(margin/2)+" pixels wide.");
		save(name, histogramImage, (256+margin), (256+margin));
}
...
//System.out.println("finished call to findChartType\n");
  ...
  public int[][] getBlobImage ( int i) {
     Region reg = allRegions[i];
     PointPixel upperleft = reg.getUpperLeft();
     PointPixel lowerright = reg.getLowerRight();
     int yb = upperleft.getRow();
     int ye = lowerright.getRow();
     int xb = upperleft.getColumn();
     int xe = lowerright.getColumn();
     int[][] blobImage = new int[ye - yb + 1][xe - xb + 1];
     int y;
     int x;
     for (y=yb;y<=ye;y++)
        for (x=xb;x<=xe;x++) {
           blobImage[y - yb][x - xb] =
              (firstPixelLabel[y][x]==reg.getRegion())? 0 : 255;
        }
     return blobImage;
  }
